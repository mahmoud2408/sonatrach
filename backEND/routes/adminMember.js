...
router.put("/member/:id", adminOnly, async (req, res) => {
  try {
    const { id } = req.params;
    const { nom, email, categorie, abonnement_expire } = req.body;
    ...
    await pool.execute(
        `UPDATE membres
        SET nom = ?, email = ?, categorie = ?, abonnement_expire = ?
       WHERE id = ?`,
        [nom, email, categorie, abonnement_expire, id]
    );

    res.status(200).json({ message: "Membre mis à jour avec succès" });
  } catch (error) {
    console.error("Erreur lors de la mise à jour du membre :", error);
    res.status(500).json({ error: "Erreur serveur lors de la mise à jour du membre" });
  }
});

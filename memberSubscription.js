...
router.post('/payer-abonnement', async (req, res) => {
  try {
    ...
    if (!user_id || !categorie) {
      return res.status(400).json({ error: "user_id et categorie sont requis." });
    }
    // Vérifier si l'utilisateur est déjà membre
    const [existing] = await pool.execute("SELECT id FROM membres WHERE user_id = ?", [user_id]);
    ...
    const [userRows] = await pool.execute("SELECT firstName, lastName, email FROM users WHERE id = ?", [user_id]);
    ...
    res.status(201).json({ message: "Abonnement payé et membre ajouté avec succès." });
  } catch (error) {
    console.error("Erreur lors du paiement de l'abonnement :", error);
    res.status(500).json({ error: "Erreur interne lors du paiement de l'abonnement." });
  }
});
